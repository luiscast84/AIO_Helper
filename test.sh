...
# Function to refresh snap package if installed
refresh_snap_package() {
    local package_name=$1
    if check_snap_package "$package_name"; then
        print_info "Found existing snap installation of $package_name, refreshing..."
        sudo snap refresh "$package_name"
        SKIPPED_PACKAGES[$package_name]="Updated via snap"
        return 0
    fi
    return 1
}
...
setup_vscode() {
    ...
    if ! command_exists code; then
        ...
        if check_snap_package "code"; then
            print_info "VS Code is installed via snap, refreshing..."
            refresh_snap_package "code"
            return 0
        }
...
install_powershell() {
    ...
    if ! command_exists pwsh; then
        # Check if PowerShell is installed via snap
        if check_snap_package "powershell"; then
            print_info "PowerShell is installed via snap, refreshing..."
            refresh_snap_package "powershell"
            return 0
        }
...
setup_helm() {
    ...
    if ! command_exists helm; then
        # Check if Helm is installed via snap
        if check_snap_package "helm"; then
            print_info "Helm is installed via snap, refreshing..."
            refresh_snap_package "helm"
            return 0
        }
...
install_k9s() {
    ...
    if ! command_exists k9s; then
        # Check if K9s is installed via snap
        if check_snap_package "k9s"; then
            print_info "K9s is installed via snap, refreshing..."
            refresh_snap_package "k9s"
            return 0
        }
...
install_mqtt_tools() {
    ...
    # Install MQTT Explorer - Check for snap installation first
    if check_snap_package "mqtt-explorer"; then
        print_info "Found existing MQTT Explorer installation via snap"
        if refresh_snap_package "mqtt-explorer"; then
            print_success "Successfully refreshed MQTT Explorer via snap"
            track_change "MQTT Explorer" "Updated via snap"
            return 0
        fi
    elif ! dpkg -l | grep -q mqtt-explorer; then
...
main() {
    ...
    if command_exists snap; then
        ...
        sudo snap refresh || print_warning "Some snap packages might not have updated properly"
    fi
    if command_exists brew; then
        brew update
    fi
